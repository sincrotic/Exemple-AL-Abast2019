...
codeunit 50004 "General Abast Library"
{
    ...
    begin
        if SIIDocUploadState.Get(Rec."Document State Id") then
            if EsSuecia(SIIDocUploadState) then
            begin
    ...
    begin
        case Rec."Document Type" of
            Rec."Document Type"::"Credit Memo" : 
                begin
                    if (Rec."Document Source" = Rec."Document Source"::"Customer Ledger") OR (Rec."Document Source" = Rec."Document Source"::"Detailed Customer Ledger") then
                    begin
                        if SalesCrMemo.get(Rec."Document No.") then
                            exit(SalesCrMemo."SII Exclude");
                        exit(false);
                    end else if (Rec."Document Source" = Rec."Document Source"::"Vendor Ledger") OR (Rec."Document Source" = Rec."Document Source"::"Detailed Vendor Ledger") then
                    begin
    ...
    begin
        case DocType of
            ...
            DocType::"Posted Return Order": begin
                if TableNo = 18 then
    ...
    begin
        ...
        IF recListaEmails.FINDSET THEN BEGIN
            REPEAT
                CASE recListaEmails."Send Type" OF
                    recListaEmails."Send Type"::Main: ToName  += ';' + recListaEmails.Email;
                    recListaEmails."Send Type"::CC:   CCName  += ';' + recListaEmails.Email;
                    recListaEmails."Send Type"::BCC:  BCCName += ';' + recListaEmails.Email;
                END;
        ...
        ELSE BEGIN
            CASE Idioma OF
                'ESP': rutaBody := rutaBodyES;
                ELSE rutaBody := rutaBodyENU;
            END;
        END;
        CompanyInfo.get;
